...
#--------------------------------------------------------------------------------------------------------
def get_color(z):
    # Color helper based on height 
    """ This function takes a height and associates it with a color array 
        to pass in the openSCAD function color() 
        (simple gradient: blue → green → brown → grey)"""
    if z < 2:
        return [0, 0.4, 1]  # Water - Blue
    elif z < 10:
        return [0, 1, 0]  # Grass - Green
    elif z < 20:
        return [0.4, 0.26, 0.13]  # Dirt - Brown
    else:
        return [0.5, 0.5, 0.5]  # Mountain - Gray
    
def minecraft(heights, highest, depths, max_depth):
    """ Returns a string to visualize a heightmap in color 3D in openSCAD in the style of minecraft """
    row = len(heights)
    col = len(heights[0])
    scad = ""
    for y in range(row):
        for x in range(col):
            ...
            color = get_color(h)
            if color == [0, 0.4, 1] and depths[y][x] < 0:
                p = abs(depths[y][x]/max_depth)
            else:
                if h <= 2:
                    color = [0, 0.4, 1]
                p = h/highest
             
            scad += f"color([{color[0]}, {color[1]}, {color[2]}]*{p:.2f})"
...
def scad(landscape, dim, highest, file="mesh.scad", adjust=0):
    ...
    plaque = f"cube([{col*x}, {row*y}, 1]);"
    name = "linear_extrude(1) {\n translate([5,5,0])\n text(\"H.I. & R-C.N IFT2125\", 1);}"
    plaque = "difference() {\n" + plaque + name + "\n}"
